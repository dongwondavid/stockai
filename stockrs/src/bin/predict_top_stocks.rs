...
fn calculate_features_for_stock_optimized(
    ...
    for feature in features {
        let value = match feature.as_str() {
            "day4_macd_histogram_increasing" => calculate_macd_histogram_increasing(db, stock_code, date).unwrap_or(0.0),
            "day4_short_macd_cross_signal" => calculate_short_macd_cross_signal(db, stock_code, date).unwrap_or(0.0),
            "day1_current_price_ratio" => calculate_current_price_ratio(db, stock_code, date).unwrap_or(1.0),
            "day1_high_price_ratio" => calculate_high_price_ratio(db, stock_code, date).unwrap_or(1.0),
            "day1_low_price_ratio" => calculate_low_price_ratio(db, stock_code, date).unwrap_or(1.0),
            "day4_open_to_now_return" => calculate_open_to_now_return(db, stock_code, date).unwrap_or(0.0),
            "day4_is_long_bull_candle" => calculate_is_long_bull_candle(db, stock_code, date).unwrap_or(0.0),
            "day1_price_position_ratio" => calculate_price_position_ratio(db, stock_code, date).unwrap_or(0.5),
            "day3_morning_mdd" => calculate_morning_mdd(db, stock_code, date).unwrap_or(0.0),
            "day1_fourth_derivative" => calculate_fourth_derivative(db, stock_code, date).unwrap_or(0.0),
            "day1_long_candle_ratio" => calculate_long_candle_ratio(db, stock_code, date).unwrap_or(0.0),
            "day1_fifth_derivative" => calculate_fifth_derivative(db, stock_code, date).unwrap_or(0.0),
            "day3_breaks_6month_high" => calculate_breaks_6month_high(daily_db, stock_code, date).unwrap_or(0.0),
            "day2_prev_day_range_ratio" => calculate_prev_day_range_ratio(daily_db, stock_code, date, trading_dates).unwrap_or(0.0),
            "day2_prev_close_to_now_ratio" => calculate_prev_close_to_now_ratio(db, daily_db, stock_code, date, trading_dates).unwrap_or(1.0),
            "day4_macd_histogram" => calculate_macd_histogram(db, stock_code, date).unwrap_or(0.0),
            "day1_sixth_derivative" => calculate_sixth_derivative(db, stock_code, date).unwrap_or(0.0),
            "day4_pos_vs_high_5d" => calculate_pos_vs_high_5d(daily_db, stock_code, date).unwrap_or(0.0),
            "day4_rsi_value" => calculate_rsi_value(db, stock_code, date).unwrap_or(50.0),
            "day4_pos_vs_high_3d" => calculate_pos_vs_high_3d(daily_db, stock_code, date).unwrap_or(0.0),
            _ => {
...
fn get_morning_data(db: &Connection, stock_code: &str, date: &str) -> Result<MorningData> {
    ...
    if table_exists == 0 {
        return Err(rusqlite::Error::InvalidParameterName(
            format!("5분봉 테이블이 존재하지 않습니다: {} (종목: {})", table_name, stock_code)
        ));
    }
    
    // 해당 날짜의 데이터 존재 여부 확인
    let data_exists: i64 = db.query_row(
    ...
    if data_exists == 0 {
        return Err(rusqlite::Error::InvalidParameterName(
            format!("9시반 이전 데이터가 없습니다: {} (종목: {}, 범위: {} ~ {})", stock_code, table_name, date_start, date_end)
        ));
    }
...
impl MorningData {
    ...
    fn get_max_high(&self) -> Option<f64> {
        self.highs.iter().fold(None, |max, &val| {
            Some(max.map_or(val, |m| m.max(val)))
        })
    }
    
    fn get_min_low(&self) -> Option<f64> {
        self.lows.iter().fold(None, |min, &val| {
            Some(min.map_or(val, |m| m.min(val)))
        })
    }
...
// 각 특징 계산 함수들 (실제 구현)
fn calculate_macd_histogram_increasing(db: &Connection, stock_code: &str, date: &str) -> Result<f64> {
    let morning_data = get_morning_data(db, stock_code, date)?;
    
    if morning_data.closes.len() >= 2 {
        let diff = morning_data.closes[morning_data.closes.len()-1] - morning_data.closes[morning_data.closes.len()-2];
...
fn calculate_fourth_derivative(db: &Connection, stock_code: &str, date: &str) -> Result<f64> {
    ...
    let last_fifth_avg = morning_data.closes[morning_data.closes.len()-fifth..].iter().sum::<f64>() / fifth as f64;
...
fn calculate_fifth_derivative(db: &Connection, stock_code: &str, date: &str) -> Result<f64> {
    ...
    let last_sixth_avg = morning_data.closes[morning_data.closes.len()-sixth..].iter().sum::<f64>() / sixth as f64;
...
fn calculate_breaks_6month_high(daily_db: &Connection, stock_code: &str, date: &str) -> Result<f64> {
    ...
    debug!("6개월 전고점 조회 - 종목: {}, 날짜 범위: {} ~ {}", stock_code, six_months_ago_str, target_date_str);
    ...
    if six_month_data_count == 0 {
        warn!("6개월 내 데이터가 없습니다. 날짜 범위: {} ~ {} (테이블: {})", six_months_ago_str, target_date_str, table_name);
        return Ok(0.0);
    }
    ...
    let six_month_high: f64 = match daily_db.query_row(
        ...
        Ok(high) => high,
        Err(e) => {
            warn!("6개월 내 최고가 조회 실패: {} (테이블: {}, 범위: {} ~ {})", e, table_name, six_months_ago_str, target_date_str);
            return Ok(0.0);
        }
    };
    ...
    if today_data_exists == 0 {
        warn!("당일 데이터가 없습니다. 날짜: {} (테이블: {})", target_date_str, table_name);
        return Ok(0.0);
    }
    ...
    let today_high: f64 = match daily_db.query_row(
        ...
        Ok(high) => high,
        Err(e) => {
            warn!("당일 고가 조회 실패: {} (테이블: {}, 날짜: {})", e, table_name, target_date_str);
            return Ok(0.0);
        }
    };
    ...
    let breaks_6month_high = if today_high >= six_month_high && six_month_high > 0.0 { 1.0 } else { 0.0 };
...
fn get_trading_dates_list(db: &Connection) -> Result<Vec<String>> {
    ...
    let table_name = if tables.contains(&"A005930".to_string()) {
        ...
        if let Some(first_table) = tables.first() {
            info!("삼성전자 테이블을 찾을 수 없어 {} 테이블을 사용합니다.", first_table);
            first_table
        } else {
            return Err(rusqlite::Error::InvalidParameterName("사용 가능한 테이블이 없습니다.".to_string()));
        }
    };
    
    // 5분봉 데이터에서 날짜만 추출 (YYYYMMDD 형식으로 변환)
    let query = format!("SELECT DISTINCT date/10000 as ymd FROM \"{}\" ORDER BY ymd", table_name);
    let mut stmt = db.prepare(&query)?;
    
    let trading_dates: Vec<String> = stmt.query_map([], |row| {
    ...
    debug!("테이블 {}에서 {}개 거래일 로드됨", table_name, trading_dates.len());
    
    if trading_dates.is_empty() {
        return Err(rusqlite::Error::InvalidParameterName(format!("테이블 {}에 거래일 데이터가 없습니다.", table_name)));
    }
...
fn get_previous_trading_day(trading_dates: &[String], date: &str) -> Result<String> {
    // 빈 배열 체크
    if trading_dates.is_empty() {
        return Err(rusqlite::Error::InvalidParameterName("거래일 리스트가 비어있습니다.".to_string()));
    }
    ...
    match current_index {
        ...
        None => {
            ...
            debug!("거래일 리스트에 {}가 없음. 가장 가까운 이전 거래일 찾는 중...", date);
            
            for trading_date in trading_dates.iter().rev() {
                if let Ok(td) = trading_date.parse::<i32>() {
                    if td < target_date {
                        debug!("가장 가까운 이전 거래일 찾음: {} (원래 기준: {})", trading_date, date);
            ...
            warn!("모든 거래일이 현재 날짜보다 큽니다. 첫 번째 거래일 사용: {}", trading_dates[0]);
            Ok(trading_dates[0].clone())
        }
    }
}

fn calculate_prev_day_range_ratio(daily_db: &Connection, stock_code: &str, date: &str, trading_dates: &[String]) -> Result<f64> {
    // 주식 시장이 열린 날 기준으로 전일 계산
    let prev_date_str = match get_previous_trading_day(trading_dates, date) {
        Ok(date) => date,
        Err(e) => {
            warn!("전일 날짜 계산 실패: {} (종목: {}, 날짜: {})", e, stock_code, date);
            return Ok(0.0);
        }
    };
    ...
    // 테이블 존재 여부 확인
    let table_exists: i64 = daily_db.query_row(
        "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name = ?",
        rusqlite::params![&table_name],
        |row| row.get(0)
    ).unwrap_or(0);
    ...
    if table_exists == 0 {
        warn!("일봉 테이블이 존재하지 않습니다: {} (종목: {})", table_name, stock_code);
        return Ok(0.0);
    }
    
    // 전일 데이터 존재 여부 확인
    let prev_data_exists: i64 = daily_db.query_row(
        &format!("SELECT COUNT(*) FROM \"{}\" WHERE date = ?", table_name),
    ...
    if prev_data_exists == 0 {
        warn!("전일 데이터가 없습니다: {} (테이블: {}, 날짜: {})", stock_code, table_name, prev_date_str);
        return Ok(0.0);
    }
    
    // 전일 고가, 저가, 종가 조회
    let (prev_high, prev_low, prev_close): (f64, f64, f64) = match daily_db.query_row(
        &format!("SELECT high, low, close FROM \"{}\" WHERE date = ?", table_name),
        rusqlite::params![&prev_date_str],
        |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?))
    ) {
        Ok(result) => result,
        Err(e) => {
            warn!("전일 데이터 조회 실패: {} (테이블: {}, 날짜: {})", e, table_name, prev_date_str);
            return Ok(0.0);
        }
    };
    
    if prev_close <= 0.0 {
        warn!("전일 종가가 유효하지 않습니다: {:.2} (종목: {})", prev_close, stock_code);
        return Ok(0.0);
    }
...
fn calculate_prev_close_to_now_ratio(db: &Connection, daily_db: &Connection, stock_code: &str, date: &str, trading_dates: &[String]) -> Result<f64> {
    // 주식 시장이 열린 날 기준으로 전일 계산
    let prev_date_str = match get_previous_trading_day(trading_dates, date) {
        Ok(date) => date,
        Err(e) => {
    ...
    // 테이블의 실제 데이터 확인 (디버깅용)
    let sample_data: Vec<String> = daily_db.prepare(&format!("SELECT date FROM \"{}\" ORDER BY date DESC LIMIT 5", table_name))?
    ...
    let prev_close: f64 = match daily_db.query_row(
        ...
        Ok(close) => close,
        Err(e) => {
            warn!("전일 종가 조회 실패: {} (테이블: {}, 날짜: {})", e, table_name, prev_date_str);
            return Ok(1.0);
        }
    };
    ...
    let morning_data = match get_morning_data(db, stock_code, date) {
        Ok(data) => data,
        Err(e) => {
            warn!("당일 9시반 이전 데이터 조회 실패: {} (종목: {})", e, stock_code);
            return Ok(1.0);
        }
    };
    ...
    if current_close <= 0.0 {
        warn!("당일 현재가가 유효하지 않습니다: {:.2} (종목: {})", current_close, stock_code);
        return Ok(1.0);
    }
...
fn calculate_pos_vs_high_5d(daily_db: &Connection, stock_code: &str, date: &str) -> Result<f64> {
    ...
    // 테이블명 (일봉 DB는 A 접두사 포함)
    let table_name = stock_code;
    debug!("5일 고점 조회 - 종목: {}, 테이블: {}", stock_code, table_name);
    ...
    let target_date_str = target_date.format("%Y%m%d").to_string();
    debug!("5일 고점 조회 범위: {} ~ {}", five_days_ago_str, target_date_str);
    ...
    let five_day_high: f64 = match daily_db.query_row(
        ...
        Ok(high) => high,
        Err(e) => {
            warn!("5일 고점 조회 실패: {} (테이블: {}, 범위: {} ~ {})", e, table_name, five_days_ago_str, target_date_str);
            return Ok(0.0);
        }
    };
...
fn calculate_pos_vs_high_3d(daily_db: &Connection, stock_code: &str, date: &str) -> Result<f64> {
    ...
    // 테이블명 (일봉 DB는 A 접두사 포함)
    let table_name = stock_code;
    debug!("3일 고점 조회 - 종목: {}, 테이블: {}", stock_code, table_name);
    ...
    let target_date_str = target_date.format("%Y%m%d").to_string();
    debug!("3일 고점 조회 범위: {} ~ {}", three_days_ago_str, target_date_str);
    ...
    let three_day_high: f64 = match daily_db.query_row(
        ...
        Ok(high) => high,
        Err(e) => {
            warn!("3일 고점 조회 실패: {} (테이블: {}, 범위: {} ~ {})", e, table_name, three_days_ago_str, target_date_str);
            return Ok(0.0);
        }
    };
...
fn load_onnx_model(model_info_path: &str) -> Result<ONNXPredictor, Box<dyn std::error::Error>> {
    ...
    if !Path::new(model_info_path).exists() {
        return Err(format!("ONNX 모델 정보 파일을 찾을 수 없습니다: {}", model_info_path).into());
    }
    ...
    if !Path::new(&model_info.onnx_model_path).exists() {
        return Err(format!("ONNX 모델 파일을 찾을 수 없습니다: {}", model_info.onnx_model_path).into());
    }
    ...
    debug!("입력 이름: {}, 출력 이름: {}", model_info.input_name, model_info.output_name);
...
fn predict_with_onnx_model(
    ...
    for (idx, stock_data) in features_data.iter().enumerate() {
        debug!("--- 종목 {} 예측 중 ({}/{}) ---", stock_data.stock_code, idx + 1, features_data.len());
        
        // 1. 특성 벡터를 f32 배열로 변환 (NaN이나 무한대 값 처리)
        let input_vec: Vec<f32> = stock_data.features.iter().map(|&x| {
            let val = x as f32;
        ...
        let input_tensor = Value::from_array(&predictor.session.allocator() as *const _ as *mut _, &input_cow)?;
        debug!("입력 텐서 생성 완료");
        ...
        let probability = if let Ok(output_tensor) = output_value.try_extract::<f32>() {
            ...
            warn!("모든 데이터 타입으로 텐서 추출 실패 (종목: {})", stock_data.stock_code);
            0.5
        };
        ...
        info!("종목 {} 예측 완료: {:.6}", stock_data.stock_code, probability);
        debug!("");
    }
...
fn get_daily_data(daily_db: &Connection, stock_code: &str, date: &str) -> Result<DailyData> {
    ...
    if table_exists == 0 {
        return Err(rusqlite::Error::InvalidParameterName(
            format!("일봉 테이블이 존재하지 않습니다: {} (종목: {})", table_name, stock_code)
        ));
    }
    
    // 해당 날짜의 데이터 존재 여부 확인
    let data_exists: i64 = daily_db.query_row(
        &format!("SELECT COUNT(*) FROM \"{}\" WHERE date = ?", table_name),
    ...
    if data_exists == 0 {
        return Err(rusqlite::Error::InvalidParameterName(
            format!("전일 데이터가 없습니다: {} (테이블: {}, 날짜: {})", stock_code, table_name, date)
        ));
    }
...
impl StockPredictor {
    ...
    fn check_daily_db_tables(db: &Connection) -> Result<(), Box<dyn std::error::Error>> {
        let tables: Vec<String> = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'")?
            .query_map([], |row| Ok(row.get::<_, String>(0)?))?
            .filter_map(|r| r.ok())
            .collect();
        ...
        if tables.contains(&"A005930".to_string()) {
            ...
            let recent_dates: Vec<String> = db.prepare("SELECT date FROM \"A005930\" ORDER BY date DESC LIMIT 5")?
                .query_map([], |row| Ok(row.get::<_, String>(0)?))?
                .filter_map(|r| r.ok())
                .collect();
    ...
    fn validate_and_get_target_date(db: &Connection, date: &str) -> Result<String, Box<dyn std::error::Error>> {
    ...
    fn predict_top_stocks(&self, date: &str) -> Result<Vec<PredictionResult>, Box<dyn std::error::Error>> {
        // 거래대금 상위 30개 종목 조회
    ...
    fn calculate_features_for_stocks(&self, stocks: &[String], date: &str) -> Result<Vec<StockFeatures>, Box<dyn std::error::Error>> {
        let mut features_data = Vec::new();
        
        for stock_code in stocks {
            match calculate_features_for_stock_optimized(&self.db, &self.daily_db, stock_code, date, &self.features, &self.trading_dates) {
                Ok(feature_values) => {
                ...
                Err(e) => {
                    warn!("종목 {} 특징 계산 실패: {} - 기본값으로 진행", stock_code, e);
    ...
    fn print_results(&self, predictions: &[PredictionResult], date: &str) {
        ...
        if let Some(best_stock) = predictions.first() {
            ...
            println!("확률: {:.4} ({:.2}%)", best_stock.probability, best_stock.probability * 100.0);
        }
    }
}
