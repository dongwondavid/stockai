...
fn main() {
    ...
    let mut daily_db = match Connection::open(&daily_db_path) {
        Ok(db) => {
            info!("DB 연결 성공");
            // 성능 최적화 설정
            let _: String = db.query_row("PRAGMA journal_mode = WAL", [], |row| row.get(0)).unwrap_or_default();
            db.execute("PRAGMA synchronous = NORMAL", []).unwrap_or_default();
            db.execute("PRAGMA cache_size = 10000", []).unwrap_or_default();
            db.execute("PRAGMA temp_store = MEMORY", []).unwrap_or_default();
            db
        },
        Err(e) => {
            error!("DB 연결 실패: {}", e);
    ...
    // DB 테이블 확인
    info!("DB 테이블 목록 확인 중...");
    let tables: Vec<String> = daily_db.prepare("SELECT name FROM sqlite_master WHERE type='table'")
        .unwrap()
    ...
    for stock_info in test_stocks {
        ...
        // 테이블 존재 여부 확인
        let table_exists: bool = daily_db.query_row(
        ...
        // 테이블 스키마 확인
        let schema: String = daily_db.query_row(
        ...
        // 컬럼 존재 여부 확인
        let columns: Vec<String> = daily_db.prepare(&format!("PRAGMA table_info({})", table_name))
            .unwrap()
        ...
        // 최근 데이터 확인
        let recent_data: Vec<(String, f64)> = daily_db.prepare(
            &format!("SELECT date, 외국인현보유비율 FROM {} ORDER BY date DESC LIMIT 5", table_name)
        )
        .unwrap()
        ...
        // 전체 데이터 범위 확인
        let date_range: Vec<(String, String)> = daily_db.prepare(
        ...
        // 전체 데이터 개수 확인
        let total_count: i64 = daily_db.query_row(
        ...
        match calculate_foreign_ratio_feature_thread_optimized(&modified_stock_info, &mut daily_db, &mut cache) {
            Ok(result) => {
                info!("결과: {}", result);
                ...
                if let Some(ratios) = cache.foreign_ratio_cache.get(&cache_key) {
                    ...
                    if ratios.is_empty() {
                        ...
                        if ratios.len() == 3 {
                            ...
                            info!("  2일전: {:.2}%", day2);
                            info!("  3일전: {:.2}%", day3);
                            info!("  연속 상승 조건: {} > {} && {} > {}", day1, day2, day2, day3);
...
fn calculate_foreign_ratio_feature_thread_optimized(
    ...
    let query = format!("SELECT date FROM {} WHERE date < ? ORDER BY date DESC LIMIT 10", stock_info.stock_code);
    ...
    for i in 0..3 {
        if i < trading_dates.len() {
            ...
            info!("조회 중: {} 테이블의 {} 날짜 (거래일)", stock_info.stock_code, trading_date_int);
            
            match daily_db.query_row(
                &format!("SELECT 외국인현보유비율 FROM {} WHERE date = ?", stock_info.stock_code),
...
fn analyze_foreign_ratio_logic_issues(ratios: &[f64]) -> Vec<String> {
    ...
    if ratios.len() != 3 {
        issues.push(format!("데이터 부족: {}일 데이터 (필요: 3일)", ratios.len()));
        return issues;
    }
    ...
    if change1 > 10.0 || change2 > 10.0 {
        issues.push(format!("급격한 외국인 현보유비율 변화 (1일 변화: {:.2}%, 2일 변화: {:.2}%)", change1, change2));
    }
    ...
    if day1 > day2 && day2 > day3 {
        ...
        if day1 <= day2 {
            issues.push(format!("✗ 최근일({:.2}%) <= 2일전({:.2}%) - 상승 중단", day1, day2));
        }
        if day2 <= day3 {
            issues.push(format!("✗ 2일전({:.2}%) <= 3일전({:.2}%) - 연속 상승 아님", day2, day3));
        }
    }
...
mod tests {
    ...
    fn test_issue_analysis() {
        ...
        let ratios = vec![10.5, 10.0, 9.5];
        let issues = analyze_foreign_ratio_logic_issues(&ratios);
        assert!(issues.iter().any(|issue| issue.contains("정상적인 3일 연속 상승 패턴")));
    }
} 