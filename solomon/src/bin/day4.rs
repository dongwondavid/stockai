...
fn main() {
    ...
    db_pool
        .solomon_db
        .execute("DROP TABLE IF EXISTS day4", [])
        .unwrap();
            ...
            match calculate_day4_features_optimized(&stock_info, &sector_manager, &db_pool, &cache)
            {
                Ok(features) => {
                    ...
                    if batch_features.len() >= BATCH_SIZE {
                        if let Err(e) =
                            save_day4_features_batch(&mut db_pool.solomon_db, &batch_features)
                        {
...
fn calculate_day4_features_optimized(
    ...
    let prev_prev_data = sector_manager.get_five_min_data(
        &db_pool.stock_db,
        &stock_info.stock_code,
        prev_prev_date,
    )?;
    ...
    let short_macd_cross_signal =
        calculate_short_macd_cross_signal(&today_data, &stock_info.stock_code, cache)?;

    // RSI 파생 특징들
    let rsi_value = calculate_rsi_value(&today_data, &stock_info.stock_code, cache)?;
    let (rsi_overbought, rsi_oversold) =
        calculate_rsi_extremes(&today_data, &stock_info.stock_code, cache)?;

    // MACD 파생 특징들
    let macd_histogram = calculate_macd_histogram(&today_data, &stock_info.stock_code, cache)?;
    let macd_histogram_increasing =
        calculate_macd_histogram_increasing(&today_data, &stock_info.stock_code, cache)?;
    ...
    // 4. 거래량 관련
    let high_volume_early_count =
        calculate_high_volume_early_count(&today_data, &stock_info.stock_code, cache)?;
    ...
    let (vwap_vs_high, vwap_vs_low) =
        calculate_vwap_vs_extremes(&today_data, &stock_info.stock_code, cache)?;
...
fn calculate_rsi_above_50(
    data: &[solomon::FiveMinData],
    stock_code: &str,
    cache: &Arc<Mutex<Cache>>,
) -> Result<i32> {
...
fn calculate_short_macd_cross_signal(
    data: &[solomon::FiveMinData],
    _stock_code: &str,
...
fn calculate_macd_histogram_increasing(
    ...
    Ok(if current_histogram > prev_histogram {
        1
    } else {
        0
    })
...
fn calculate_pos_vs_high_3d(
    today_data: &[solomon::FiveMinData],
    prev_data: &[solomon::FiveMinData],
    prev_prev_data: &[solomon::FiveMinData],
) -> f64 {
...
fn calculate_high_volume_early_count(
    data: &[solomon::FiveMinData],
    _stock_code: &str,
    _cache: &Arc<Mutex<Cache>>,
) -> Result<i32> {
    ...
    let morning_avg_volume =
        morning_data.iter().map(|d| d.volume).sum::<i32>() as f64 / morning_data.len() as f64;
...
fn calculate_bollinger_features(
    data: &[solomon::FiveMinData],
    stock_code: &str,
    cache: &Arc<Mutex<Cache>>,
) -> Result<(f64, f64, i32)> {
    ...
    // 캐시에 저장
    {
        ...
        cache_guard
            .bollinger_cache
            .insert(cache_key, (upper_band, sma, lower_band));
    }
...
fn get_stock_list_from_answer(db: &Connection) -> Result<Vec<Day4StockInfo>> {
    ...
    while let Some(row) = rows.next()? {
        ...
        stock_list.push(Day4StockInfo { stock_code, date });
    }
    Ok(stock_list)
}
