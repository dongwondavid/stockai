...
fn main() {
    ...
    let daily_tables: Vec<String> = db_pool.daily_db.prepare("SELECT name FROM sqlite_master WHERE type='table'")
        .unwrap()
    ...
    let stock_tables: Vec<String> = db_pool.stock_db.prepare("SELECT name FROM sqlite_master WHERE type='table'")
        .unwrap()
        .query_map([], |row| row.get(0))
        .unwrap()
        .filter_map(|r| r.ok())
    ...
    for stock_info in test_stocks {
        ...
        // 일봉 테이블 존재 여부 확인
        let daily_table_exists: bool = db_pool.daily_db.query_row(
        ...
        // 5분봉 테이블 존재 여부 확인
        let stock_table_exists: bool = db_pool.stock_db.query_row(
            "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name = ?",
            rusqlite::params![table_name],
            |row| row.get(0)
        ).unwrap_or(false);
        ...
        // 일봉 테이블 스키마 확인
        let daily_schema: String = db_pool.daily_db.query_row(
        ...
        // 일봉 컬럼 존재 여부 확인
        let daily_columns: Vec<String> = db_pool.daily_db.prepare(&format!("PRAGMA table_info({})", table_name))
            .unwrap()
        ...
        // 5분봉 컬럼 존재 여부 확인
        let stock_columns: Vec<String> = db_pool.stock_db.prepare(&format!("PRAGMA table_info({})", table_name))
            .unwrap()
            .query_map([], |row| row.get(1))
            .unwrap()
            .filter_map(|r| r.ok())
        ...
        // 일봉 데이터 범위 확인
        let daily_date_range: Vec<(String, String)> = db_pool.daily_db.prepare(
            &format!("SELECT MIN(date), MAX(date) FROM {}", table_name)
        )
        .unwrap()
        ...
        // 일봉 데이터 샘플 확인 (최근 5개)
        let daily_sample: Vec<(String, f64, f64, f64, f64)> = db_pool.daily_db.prepare(
            &format!("SELECT date, open, high, low, close FROM {} ORDER BY date DESC LIMIT 5", table_name)
        )
        .unwrap()
        ...
        for (date, open, high, low, close) in &daily_sample {
            info!("  {}: O={:.2}, H={:.2}, L={:.2}, C={:.2}", date, open, high, low, close);
        }
        
        // 특정 날짜 데이터 확인 (테스트용)
        let test_date = "20240115";
        let specific_date_data: Vec<(String, f64, f64, f64, f64)> = db_pool.daily_db.prepare(
            &format!("SELECT date, open, high, low, close FROM {} WHERE date = ?", table_name)
        )
        .unwrap()
        .query_map([test_date], |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?, row.get(3)?, row.get(4)?)))
        .unwrap()
        .filter_map(|r| r.ok())
        .collect();
        
        if let Some((date, open, high, low, close)) = specific_date_data.first() {
            info!("특정 날짜 {} 데이터: O={:.2}, H={:.2}, L={:.2}, C={:.2}", date, open, high, low, close);
        } else {
            info!("특정 날짜 {} 데이터가 없습니다.", test_date);
        }
        
        // 5분봉 데이터 범위 확인
        let stock_date_range: Vec<(String, String)> = db_pool.stock_db.prepare(
            &format!("SELECT MIN(date), MAX(date) FROM {}", table_name)
        )
        .unwrap()
        .query_map([], |row| Ok((row.get(0)?, row.get(1)?)))
        ...
        info!("섹터 캐시 계산 중...");
        let date_cache = match sector_manager.calculate_date_sector_cache(&stock_info.date, &db_pool.solomon_db) {
            Ok(cache) => {
                info!("섹터 캐시 계산 완료: {}개 종목 정보", cache.stock_info.len());
                cache
            },
            Err(e) => {
                error!("섹터 캐시 계산 실패: {}", e);
        ...
        match calculate_high_break_features_thread_optimized(&modified_stock_info, &mut db_pool.daily_db, &sector_manager, &date_cache, &today_data) {
            Ok((breaks_6month_high, breaks_6month_high_with_long_candle, breaks_6month_high_with_leading_sector)) => {
                ...
                info!("2. 6개월 전고점 돌파 + 장대양봉: {}", breaks_6month_high_with_long_candle);
                info!("3. 6개월 전고점 돌파 + 장대양봉 + 주도 섹터: {}", breaks_6month_high_with_leading_sector);
...
fn calculate_high_break_features_thread_optimized(
    ...
    info!("테이블 {} 전체 데이터 개수: {}", stock_info.stock_code, table_count);
    ...
    info!("쿼리 파라미터: {} ~ {}", six_months_ago_str, target_date_str);
    
    // 6개월 내 데이터 개수 확인
    let six_month_data_count: i64 = daily_db.query_row(
        &format!("SELECT COUNT(*) FROM {} WHERE date >= ? AND date < ?", stock_info.stock_code),
    ...
    // 당일 일봉 고가 조회
    let today_high_query = format!(
        "SELECT high FROM {} WHERE date = ?", 
        stock_info.stock_code
    );
    ...
    let today_data_exists: i64 = daily_db.query_row(
        &format!("SELECT COUNT(*) FROM {} WHERE date = ?", stock_info.stock_code),
        rusqlite::params![target_date_str],
        |row| row.get(0)
    ).unwrap_or(0);
    
    info!("당일 데이터 존재 여부: {} (1: 존재, 0: 없음)", today_data_exists);
    ...
    let breaks_6month_high_with_leading_sector = {
        if breaks_6month_high_with_long_candle == 1 {
            ...
            info!("주도 섹터 여부: {} (상위 3개 섹터: {:?})", 
                is_leading_sector, 
                sorted_sectors.iter().take(3).map(|(s, _)| s).collect::<Vec<_>>()
            );
    ...
    info!("6개월 전고점 돌파 + 장대양봉 + 주도 섹터: {}", breaks_6month_high_with_leading_sector);
    ...
    let _analysis = format!(
        ...
        주도 섹터: {} (상위 3개 섹터 내 포함)\n\
        최종 결과: ({}, {}, {})",
        stock_info.date.format("%Y-%m-%d"), target_date_str,
        six_months_ago.format("%Y-%m-%d"), six_months_ago_str,
        ...
        breaks_6month_high, breaks_6month_high_with_long_candle, breaks_6month_high_with_leading_sector
    );
...
mod tests {
    ...
    fn test_high_break_logic() {
        ...
        let breaks_high = if today_high >= six_month_high && six_month_high > 0.0 { 1 } else { 0 };
        ...
        // 복합 조건 테스트
        let combined = if breaks_high == 1 && is_long_candle == 1 { 1 } else { 0 };
