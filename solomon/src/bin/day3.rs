...
fn main() {
    ...
    let sector_cache = Arc::new(calculate_all_sector_cache_with_progress(
        &date_groups,
        &sector_manager,
        &_main_db,
    ));
    ...
    info!(
        "Day3 특징 계산을 시작합니다. 총 {}개 종목을 병렬 처리합니다.",
        total_stocks
    );
            ...
            Ok(features) => {
                for (stock_info, feature) in features {
                    ...
                    if batch_features.len() >= BATCH_SIZE {
                        if let Err(e) =
                            save_day3_features_batch_optimized(&mut _main_db, &batch_features)
                        {
...
fn calculate_all_sector_cache_with_progress(
    ...
    info!(
        "섹터 정보 병렬 계산 완료: {}개 날짜 (성공: {}개), 총 소요시간: {:.2?}, 평균: {:.2}ms/날짜",
        processed_count,
        success_count,
        total_elapsed,
        total_elapsed.as_millis() as f64 / processed_count as f64
    );
...
fn process_date_group_optimized(
    ...
    for stock_info in stocks {
        ...
        match calculate_day3_features_thread_optimized(
            &stock_info,
            sector_manager,
            &mut resources,
            date_cache,
...
fn calculate_day3_features_thread_optimized(
    ...
    let today_data = get_five_min_data_cached_thread(
    ...
    ) = calculate_high_break_features_thread_optimized(
        &stock_info,
        &resources.daily_db,
        sector_manager,
        date_cache,
    ...
    let (was_prev_answer, was_prev_answer_sector_first, prev_answer_sector_rank_ratio) =
        calculate_prev_answer_features_thread_optimized(
            &stock_info,
            &resources.solomon_db,
            sector_manager,
        )?;
...
fn calculate_prev_answer_features_thread_optimized(
    ...
    // 전날 30위 내에 포함되었는지 확인
    let was_prev_answer = if prev_date_cache
        .stock_info
        .iter()
        .any(|(code, _)| code == &stock_info.stock_code)
    {
        1
    } else {
        0
...
fn calculate_high_volume_gain_experience_thread_optimized(
    ...
    let count: i32 = daily_db
        .query_row(
            &format!(
                "SELECT COUNT(*) FROM {} WHERE date >= ? AND date < ? 
         AND volume * close >= 1000000000000 AND (close - open) / open >= 0.1",
                table_name
            ),
            rusqlite::params![six_months_ago_str, target_date_str],
            |row| row.get(0),
...
fn calculate_ema_alignment_thread_optimized(
    ...
    let prices: Vec<f64> = daily_db
        ...
        .query_map(rusqlite::params![target_date_str], |row| row.get(0))?
        .filter_map(|r| r.ok())
        .collect();
...
fn calculate_morning_volume_ratio_thread_optimized(
    ...
    for i in 1..=5 {
        ...
        let data = get_five_min_data_cached_thread(
            &stock_info.stock_code,
            date,
            sector_manager,
            &resources.stock_db,
    ...
    let avg_volume = if day_count > 0 {
        total_volume as f64 / day_count as f64
    } else {
        0.0
    };
...
fn get_stock_list_from_answer() -> Result<Vec<Day3StockInfo>> {
    ...
    let mut stmt =
        db.prepare("SELECT stock_code, date FROM answer_v3 WHERE date < 20230601 ORDER BY date")?;
    ...
    while let Some(row) = rows.next()? {
        ...
        stock_list.push(Day3StockInfo { stock_code, date });
    }
    Ok(stock_list)
}
