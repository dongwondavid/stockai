...
fn get_sample_stocks(db: &Connection) -> Result<Vec<Day4StockInfo>> {
    let mut stmt = db.prepare("SELECT stock_code, date FROM answer ORDER BY date DESC LIMIT 5")?;
...
fn analyze_macd(data: &[solomon::FiveMinData], _stock_code: &str) -> Result<()> {
    ...
    println!(
        "MACD > Signal: {}",
        if macd_regular > signal_regular {
            "예"
        } else {
            "아니오"
        }
...
fn analyze_other_features(data: &[solomon::FiveMinData]) {
    ...
    // 장대양봉 여부
    let is_long_bull = if open_price > 0.0 && return_rate >= 0.01 {
        "예"
    } else {
        "아니오"
    };
