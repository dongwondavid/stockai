...
fn main() {
    ...
    db_pool
        .solomon_db
        .execute("DROP TABLE IF EXISTS day1", [])
        .unwrap();
    ...
    for (date, stocks) in date_groups {
        ...
        for stock_info in stocks {
            ...
            match calculate_day1_features_optimized(
                &stock_info,
                &sector_manager,
                &db_pool,
                &date_cache,
                ...
                Ok(features) => {
                    ...
                    if batch_features.len() >= BATCH_SIZE {
                        if let Err(e) =
                            save_day1_features_batch(&mut db_pool.solomon_db, &batch_features)
                        {
...
fn calculate_day1_features_optimized(
    ...
    // 1. 오늘의 시가 기준
    let (current_price_ratio, high_price_ratio, low_price_ratio) =
        calculate_price_ratios(&today_data);

    // 2. 당일 가격 위치 관련
    let (price_position_ratio, vwap_position_ratio) =
        calculate_position_ratios_optimized(&today_data, &stock_info.stock_code, cache)?;

    // 3. 동시 상승 종목 관련 (15위 내)
    let (same_sector_rising_count_15, has_3_or_more_rising_15) =
        calculate_sector_rising_features(date_cache, &stock_info.stock_code, 15);

    // 4. 동시 상승 종목 관련 (30위 내)
    let (same_sector_rising_count_30, has_3_or_more_rising_30) =
        calculate_sector_rising_features(date_cache, &stock_info.stock_code, 30);

    // 5. 업종 내 상대적 강도
    let (sector_rank_ratio, is_sector_first) =
        calculate_sector_rank_features(date_cache, &stock_info.stock_code);

    // 6. 거래량 관련
    let volume_ratio =
        calculate_volume_ratio_optimized(&today_data, &stock_info.stock_code, cache)?;
    ...
    let (
        ...
        fifth_derivative,
        sixth_derivative,
    ) = calculate_derivatives(&today_data);
...
fn calculate_sector_rising_features(
    date_cache: &DateSectorCache,
    stock_code: &str,
    limit: usize,
) -> (i32, i32) {
...
fn calculate_position_ratios_optimized(
    data: &[FiveMinData],
    stock_code: &str,
    cache: &Arc<Mutex<Cache>>,
) -> Result<(f64, f64)> {
...
fn calculate_volume_ratio_optimized(
    ...
    if let Some(&avg_volume) = cache_guard.volume_cache.get(&cache_key) {
        return Ok(if avg_volume > 0.0 {
            data.iter().map(|d| d.volume as f64).sum::<f64>() / avg_volume
        } else {
            0.0
        });
    }
    ...
    let ratio = if falling_avg > 0.0 {
        rising_avg / falling_avg
    } else {
        0.0
    };
...
fn get_stock_list_from_answer(db: &Connection) -> Result<Vec<Day1StockInfo>> {
    ...
    while let Some(row) = rows.next()? {
        ...
        stock_list.push(Day1StockInfo { stock_code, date });
    }
    Ok(stock_list)
}
...
fn calculate_position_ratios(data: &[FiveMinData]) -> (f64, f64) {
    ...
    // VWAP 계산
    let total_volume_price: f64 = data
        .iter()
        .map(|d| (d.close as f64) * (d.volume as f64))
        .sum();
    ...
    // VWAP 대비 위치 비율
    let vwap_position_ratio = if vwap > 0.0 {
