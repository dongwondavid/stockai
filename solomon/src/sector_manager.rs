...
impl SectorManager {
    ...
    pub fn load_from_csv(&mut self, csv_path: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for line in reader.lines() {
            ...
            if parts.len() >= 3 {
                ...
                debug!("종목 정보 로드: 코드={}, 이름={}, 섹터={}", code, name, sector);
                ...
                warn!("잘못된 CSV 라인 {}: {} (파트 수: {})", line_count, line, parts.len());
            }
        }

        info!("섹터 정보 로드 완료: {}개 종목", loaded_count);
        debug!("로드된 종목 코드들: {:?}", self.stock_map.keys().take(10).collect::<Vec<_>>());
        Ok(())
    }
    ...
    pub fn get_stock_info(&self, code: &str) -> StockInfo {
        ...
        debug!("종목 정보 조회: 원본코드={}, 정리된코드={}", code, clean_code);
        ...
        if let Some(stock_info) = self.stock_map.get(clean_code) {
            ...
            debug!("섹터 정보를 찾을 수 없는 종목코드: {} (기타로 설정)", clean_code);
            debug!("사용 가능한 종목 코드들 (처음 10개): {:?}", 
    ...
    pub fn get_sector_rank(&self, target_code: &str, stock_codes: &[String]) -> Option<usize> {
        ...
        debug!("섹터 순위 계산: 타겟종목={}, 타겟섹터={}", target_code, target_sector);
        ...
        // 타겟 종목의 순위 찾기
        let rank = sorted_stocks.iter().position(|code| code == target_code)
            .map(|pos| pos + 1);
    ...
    pub fn calculate_date_sector_cache(
        ...
        let mut stmt = db.prepare(
            "SELECT stock_code FROM answer WHERE date = ? ORDER BY rank"
        )?;
        
        let all_stocks: Vec<String> = stmt.query_map([&date_int], |row| {
            Ok(row.get(0)?)
        })?
        .filter_map(|r| r.ok())
        .collect();
        
        debug!("날짜 {}의 총 종목 수: {}", date.format("%Y-%m-%d"), all_stocks.len());
        ...
        let stock_db = match Connection::open(&stock_db_path) {
            ...
            Err(e) => {
                return Err(rusqlite::Error::InvalidPath(
                    format!("주가 데이터베이스 연결 실패: {} (경로: {})", e, stock_db_path).into()
                ));
            }
        };
        ...
        for (rank, code) in all_stocks.iter().enumerate() {
            ...
            sector_stocks.entry(sector.clone()).or_insert_with(Vec::new).push((code.clone(), gain_ratio));
        }
        ...
        for (sector, stocks) in sector_stocks {
            ...
            sorted_stocks.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal)); // 내림차순 정렬 (상승률 높은 순)
            let ranked_codes: Vec<String> = sorted_stocks.into_iter().map(|(code, _)| code).collect();
    ...
    /// 5분봉 데이터 가져오기
    pub fn get_five_min_data(&self, db: &Connection, stock_code: &str, date: NaiveDate) -> Result<Vec<FiveMinData>> {
        ...
        let data = stmt.query_map([&date_start, &date_end], |row| {
            Ok(FiveMinData {
                date: row.get(0)?,
                open: row.get(1)?,
                high: row.get(2)?,
    ...
    pub fn calculate_sector_rising_features_15(
        ...
        let current_sector = date_cache.stock_info.get(stock_code)
            .map(|(_, sector)| sector.clone())
            .unwrap_or_else(|| "기타".to_string());
        
        let same_sector_rising_count_15 = date_cache.sector_rising_count_15.get(&current_sector).copied().unwrap_or(0);
        let has_3_or_more_rising_15 = if same_sector_rising_count_15 >= 3 { 1 } else { 0 };
    ...
    pub fn calculate_sector_rising_features_30(
        ...
        let same_sector_rising_count_30 = date_cache.sector_rising_count_30.get(&current_sector).copied().unwrap_or(0);
        let has_3_or_more_rising_30 = if same_sector_rising_count_30 >= 3 { 1 } else { 0 };
    ...
    pub fn calculate_sector_strength(
        ...
        if let Some(stocks) = sector_ranked_stocks {
            let rank = stocks.iter().position(|code| code == stock_code)
                .map(|pos| pos + 1)
                .unwrap_or(0) as f64;
    ...
    pub fn calculate_sector_rising_features_realtime_15(
        ...
        let mut stmt = db.prepare(
            "SELECT stock_code FROM answer WHERE date = ? ORDER BY rank LIMIT 15"
        )?;
        
        let top_15_stocks: Vec<String> = stmt.query_map([&date_int], |row| {
            Ok(row.get(0)?)
        })?
        .filter_map(|r| r.ok())
        .collect();
    ...
    pub fn calculate_sector_rising_features_realtime_30(
        ...
        let mut stmt = db.prepare(
            "SELECT stock_code FROM answer WHERE date = ? ORDER BY rank LIMIT 30"
        )?;
        
        let top_30_stocks: Vec<String> = stmt.query_map([&date_int], |row| {
            Ok(row.get(0)?)
        })?
        .filter_map(|r| r.ok())
        .collect();
    ...
    pub fn get_db_table_list(&self, db: &Connection) -> Result<Vec<String>> {
        ...
        debug!("테이블 목록 (처음 10개): {:?}", tables.iter().take(10).collect::<Vec<_>>());
